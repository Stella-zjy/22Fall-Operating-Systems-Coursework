...
void printFinalStatistics(task tasks[], int nbOfTasks, int totalTime) {
    ...
    for(i=0; i<nbOfTasks; i++) {
        ...
        totalWaitingTime += turnaroundTime - tasks[i].computationTime;
        penaltyRate = turnaroundTime/tasks[i].computationTime;
        printf("Task: %s \t turnaround time:%d \t penalty rate:%2.2f\n",
               tasks[i].name, turnaroundTime, penaltyRate);
    }
    averageWaitingTime = (float)totalWaitingTime/nbOfTasks;
    throughput = (float)nbOfTasks/totalTime;
...
int main(int argc, char *argv[]){
    ...
    // Modified to accomodate for policy IORR
    printf("Loading file of tasks\n");
    if (strcmp(argv[2], "IORR") == 0){
        while (fgets(line, sizeof(line), file) != NULL ){
            sscanf(line, "%s %u %u %u %u\n", tasks[nbOfTasks].name, &(tasks[nbOfTasks].computationTime), &(tasks[nbOfTasks].arrivalDate), &(tasks[nbOfTasks].IOTime), &(tasks[nbOfTasks].IORate));
